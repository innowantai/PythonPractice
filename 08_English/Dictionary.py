...
def combineData(search,Exp,flag,kk):
...
def CallPastSaveName():
    try:        
        with open('saveName.txt','r') as f:
            SaveName.set(f.readline())
    except:
        pass
def SaveNowName():
    with open('saveName.txt','w') as f:
        f.write(SaveName.get())
        
def main():
    search = Input.get()  
    FileName = SaveName.get() + '.xlsx'
    dd = DictionaryProcess(search)  
    Exp,flag = dd.getExplain()  
    kk = dd.getKK() 
    data = combineData(search,Exp,flag,kk)  
...
Input_Fra.pack() 
lab = tk.Label(Input_Fra,text = '儲存檔名 : ',font = ('微軟正黑體',12))
lab.grid(row = 0,column = 0, padx = 5)

txtInput = tk.Entry(Input_Fra, textvariable = SaveName ,font = ('times new roman',12),width = 12)
txtInput.grid(row=0, column=1,padx = 5)

lab = tk.Label(Input_Fra,text = '查詢單字 : ',font = ('微軟正黑體',12))
lab.grid(row = 0,column = 2, padx = 5)

txtInput = tk.Entry(Input_Fra, textvariable = Input ,font = ('times new roman',12))
txtInput.grid(row=0, column=3,pady = 5)

btnEnter = tk.Button(Input_Fra,text = '確認',height = 1, command = main, font = ('微軟正黑體',12))
btnEnter.grid(row=0, column=4, padx=10,pady = 5)
